...
const createTables = async (): Promise<void> => {
  try {
    ...
    // Styles table (catalog of styles)
    await db.execAsync(`
    ...
    // Customer styles table (styles saved per-customer)
    await db.execAsync(`
    ...
    // Customer measurements table (measurements saved per customer)
    await db.execAsync(`
...
const handleMigrations = async (): Promise<void> => {
  try {
    if (!db) {
      throw new Error("Database not initialized");
    }
    ...
    const tableExists: any = await db.getFirstAsync(`
...
const ensureDbInitialized = async (): Promise<any> => {
  try {
    if (!db) {
...
export const OrderStorage = {
  ...
  async saveOrder(orderData: any): Promise<any> {
    try {
      ...
      // Allow reusing an existing customer id if provided (e.g., save-and-next flow)
      const customerId = orderData.customer?.customerId || `CUST-${Date.now()}`;
      ...
      await db.withTransactionAsync(async () => {
        try {
          // Insert or update customer (use provided customerId if available)
          await db.runAsync(
          ...
          // Insert suit images
          if (orderData.order.suitImages && Array.isArray(orderData.order.suitImages)) {
            console.log(`Inserting ${orderData.order.suitImages.length} suit images...`);
            for (const image of orderData.order.suitImages) {
              if (image.uri && image.id) {
                try {
                  ...
                  console.error(`Error inserting suit image ${image.id}:`, imageError);
                  throw new Error(`Failed to save suit image: ${imageError.message}`, {
                    cause: imageError,
                  });
                }
              }
  ...
  async getAllOrders(): Promise<any[]> {
    try {
      ...
      for (const row of result) {
        try {
          const measurements: any[] = await database.getAllAsync(
            "SELECT * FROM measurements WHERE order_id = ? ORDER BY id",
            [row.order_id]
          );

          const suitImages: any[] = await database.getAllAsync(
          ...
          console.error("Error processing measurements for order:", row.order_id, measurementError);
  ...
  async updateOrderStatus(orderId: string, status: string): Promise<any> {
    try {
      ...
      const now = new Date().toISOString();
      await db.runAsync("UPDATE orders SET status = ?, updated_at = ? WHERE order_id = ?", [
        status,
        now,
        orderId,
      ]);
    ...
    try {
      ...
      await db.withTransactionAsync(async () => {
        try {
          ...
          await db.runAsync("DELETE FROM measurements WHERE order_id = ?", [orderId]);

          // Delete suit images (cascade should handle this, but being explicit)
          await db.runAsync("DELETE FROM suit_images WHERE order_id = ?", [orderId]);
  ...
  async searchOrdersByCustomer(customerName: string): Promise<any[]> {
    try {
      await ensureDbInitialized();
      ...
      const orders = [];
      for (const row of result) {
        const measurements: any[] = await db.getAllAsync(
          "SELECT * FROM measurements WHERE order_id = ? ORDER BY id",
          [row.order_id]
        );
...
export const DraftStorage = {
  ...
  async hasDraft(): Promise<boolean> {
    try {
      await ensureDbInitialized();
      const result: any = await db.getFirstAsync("SELECT COUNT(*) as count FROM draft_orders");
...
export const CustomerStorage = {
  ...
  async getCustomerById(customerId: string): Promise<any> {
    try {
      ...
      const result: any = await db.getFirstAsync("SELECT * FROM customers WHERE customer_id = ?", [
        customerId,
      ]);
  ...
  // Insert new customer (or replace existing)
  async insertCustomer(customerId: string, customerData: any): Promise<any> {
    try {
      await ensureDbInitialized();
  ...
  async deleteCustomer(customerId: string): Promise<boolean> {
    try {
      ...
      await db.withTransactionAsync(async () => {
        try {
          // Delete customer
          await db.runAsync("DELETE FROM customers WHERE customer_id = ?", [customerId]);
        } catch (transactionError) {
  ...
  // Save measurements for a customer (replace existing measurements)
  async saveCustomerMeasurements(customerId: string, measurements: any[]): Promise<any> {
    try {
      await ensureDbInitialized();
      ...
      await db.withTransactionAsync(async () => {
        // Delete existing measurements for customer
        await db.runAsync("DELETE FROM customer_measurements WHERE customer_id = ?", [customerId]);

        if (measurements && Array.isArray(measurements)) {
          for (const m of measurements) {
            await db.runAsync(
              `INSERT INTO customer_measurements (customer_id, en_name, ur_name, value, image_uri, created_at)
               VALUES (?, ?, ?, ?, ?, ?)`,
              [
                customerId,
                m.enName || m.en_name || "",
  ...
  async getMeasurementsByCustomer(customerId: string): Promise<any[]> {
    try {
      ...
      const rows: any[] = await db.getAllAsync(
        "SELECT * FROM customer_measurements WHERE customer_id = ? ORDER BY id",
        [customerId]
      );
      ...
      console.error("Error getting customer measurements:", error);
      return [];
    }
  },

  // Delete measurements for a customer
  async deleteMeasurementsByCustomer(customerId: string): Promise<boolean> {
    try {
      await ensureDbInitialized();
...
export const StorageUtils = {
  ...
  async testDatabaseConnection(): Promise<boolean> {
    try {
      ...
      // Simple test query
      const result: any = await database.getFirstAsync("SELECT 1 as test");
  ...
  async testDatabase(): Promise<any> {
    try {
      ...
      // Test basic query
      const result: any = await db.getFirstAsync("SELECT 1 as test");
  ...
  async getStorageInfo(): Promise<any> {
    try {
      ...
      const ordersCount: any = await db.getFirstAsync("SELECT COUNT(*) as count FROM orders");
      const customersCount: any = await db.getFirstAsync("SELECT COUNT(*) as count FROM customers");
      const pendingCount: any = await db.getFirstAsync(
        "SELECT COUNT(*) as count FROM orders WHERE status = 'pending'"
      );
      const completedCount: any = await db.getFirstAsync(
